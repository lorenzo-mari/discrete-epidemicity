...
pathogen_pars_ave=(pathogen_pars_min+pathogen_pars_max)/2;
pathogen_pars_ave=pathogen_pars_ave(:,ord);

I=pathogen_pars_ave(1,:);
theta=pathogen_pars_ave(2,:);
pL=pathogen_pars_ave(3,:);
pN=pathogen_pars_ave(4,:);
pA=pathogen_pars_ave(5,:);
qL=pathogen_pars_ave(6,:);
qN=pathogen_pars_ave(7,:);
qA=pathogen_pars_ave(8,:);
rA=pathogen_pars_ave(9,:);
...
tick_pars_ave=(tick_pars_min+tick_pars_max)/2;
tick_pars_ave=[repmat(tick_pars_ave(:,1),1,3) tick_pars_ave(:,2:end)];
tick_pars_ave=tick_pars_ave(:,ord);
...
cLN=tick_pars_ave(8,:);
cNN=tick_pars_ave(9,:);
cAN=tick_pars_ave(10,:);
cLA=tick_pars_ave(11,:);
cNA=tick_pars_ave(12,:);
cAA=tick_pars_ave(13,:);
cS=tick_pars_ave(14,:);
nLH=tick_pars_ave(15,:);
nNH=tick_pars_ave(16,:);
nAH=tick_pars_ave(17,:);
dL=tick_pars_ave(18,:);
dN=tick_pars_ave(19,:);
dA=tick_pars_ave(20,:);
hCN=tick_pars_ave(21,:);
hCS=tick_pars_ave(22,:);
...
for p=1:np
    ...
    for i=1:nc
        ...
        for k=1:nf
            ...
            if nonsystemic==1
                K(2:4,1:3)=K(2:4,1:3)*f(k);
            end
    
            Rc(p,i,k)=eigs(K,1,'largestabs');
            Ec(p,i,k)=max(sum(K));
        end    
    end
end
...
for p=1:np
    for i=1:nc
        fR_ind=find(Rc(p,i,:)<1,1,'last');
        if ~isempty(fR_ind)
            fR(p,i)=f(fR_ind);
        end
        fE_ind=find(Ec(p,i,:)<1,1,'last');
        if ~isempty(fE_ind)
            fE(p,i)=f(fE_ind);
            Rstar(p,i)=Rc(p,i,fE_ind);
        end
    end
end
...
nexttile(1)
bar(1:np,R0)
set(gca,'XLim',[-0.25 np+1.25],'XTick',1:np,'XTickLabel',[],'YLim',[0 10],'YTick',0:2:10,'TickDir','out','TickLabelInterpreter','latex')
...
nexttile(2)
bar(1:np,E0)
set(gca,'XLim',[-0.25 np+1.25],'XTick',1:np,'XTickLabel',[],'YScale','log','YLim',[1 1e4],'YTick',10.^(0:4),'TickDir','out','TickLabelInterpreter','latex')
...
nexttile(3)
bar(1:np,Rstar(:,end))
set(gca,'XLim',[-0.25 np+1.25],'XTick',1:np,'XTickLabel',pathogen,'YScale','log','YLim',[1e-3 1],'YTick',[1e-3 1e-2 0.1 1],'TickDir','out','TickLabelInterpreter','latex')
ylabel('RN to prevent epidemicity, $\mathcal{R}_*$','Interpreter','latex')
...
nexttile(4)
bar(1:np,[HIT(:,end) HET(:,end)],1)
set(gca,'XLim',[-0.25 np+1.25],'XTick',1:np,'XTickLabel',pathogen,'YLim',[0 1.2],'YTick',0:0.2:1,'TickDir','out','TickLabelInterpreter','latex')
ylabel({'HIT and HET'},'Interpreter','latex')
...
set(gcf,'NumberTitle','off','Name','Figure 3')

%%
figure('Renderer','painters','Units','centimeters','Position',[40 10 17.8 12])
t=tiledlayout(2,3,'TileSpacing','compact','Padding','compact');
lbl={'(a)','(b)','(c)','(d)','(e)','(f)',};

for i=1:nc-1
    nexttile(i)
    bar(1:np,[HIT(:,i) HET(:,i)],1)

    ax=gca;
    set(ax,'XLim',[-0.25 np+1.25],'XTick',1:np,'YLim',[0 1.2],'YTick',0:0.2:1,'TickDir','out','TickLabelInterpreter','latex')
    if i<=3
        set(ax,'XTickLabel',[])
    else
        set(ax,'XTickLabel',pathogen)
        ax.XAxis.Label.FontSize=10;
    end
    title(lbl{i},'Interpreter','latex','Units','normalized','Position', [0.05, 0.9, 0],'FontSize',10)
    leg=legend('HIT','HET','Location','north','Box','off','NumColumns',2,'Interpreter','latex','FontSize',10);
    leg.ItemTokenSize=[12 12];
    ax.XAxis.FontSize=8;
    ax.YAxis.FontSize=8;
...
ylabel(t,{'Herd immunity/epidemicity threshold'},'Interpreter','latex','FontSize',10)

set(gcf,'NumberTitle','off','Name','Figure S11')
